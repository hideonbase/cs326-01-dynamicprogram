...
import {getUserId, getUserName, logout, removeAllChildNodes} from "./common.js";
...
function toggle_fav(e) {
    ...
    const permission = {customerId: getUserId()}
...
window.onload = function () {
    ...
    if (userId) {
        fetch('/house/house?id=' + userId).then(value => value.json()).then(render_house_list).catch(reason => {
            alert("Server Error");
        });
    } else {
...
searchButton.addEventListener('click', function (e) {
    ...
    if (keyword) {
        ...
        let url = '/house/search?keyword=' + encodeURIComponent(keyword);
        if (userId) {
            url += '&id=' + userId;
        }
        fetch(url).then(value => value.json()).then(render_house_list).catch(reason => {
            alert("Server Error");
        });
    } else {
